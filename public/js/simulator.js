...
async function startLiveRecording() { if (!navigator.mediaDevices?.getUserMedia || !window.MediaRecorder) { refs.liveRecordingStatus.textContent = "LIVE MICROPHONE NOT SUPPORTED IN THIS BROWSER"; return; } try { discardLiveRecording = false; liveRecordingStream = await navigator.mediaDevices.getUserMedia({ audio: true }); liveRecordingChunks = []; liveRecorder = new MediaRecorder(liveRecordingStream); liveRecorder.ondataavailable = (event) => { if (event.data.size) liveRecordingChunks.push(event.data); }; liveRecorder.onstop = () => { const shouldDiscard = discardLiveRecording; const blob = new Blob(liveRecordingChunks, { type: liveRecorder.mimeType || "audio/webm" }); liveRecordingBlob = blob; if (!shouldDiscard) { if (liveRecordingUrl) URL.revokeObjectURL(liveRecordingUrl);
  liveRecordingUrl = URL.createObjectURL(blob); const audio = document.querySelector('audio[data-audio="emergency"]'); audio.src = liveRecordingUrl; audio.load(); refs.liveRecordingCard.classList.remove("hidden"); refs.liveRecordingTitle.textContent = "Live emergency recording"; refs.liveRecordingDuration.textContent = `${Math.max(1, Math.round(blob.size / 1000))} KB`; refs.liveRecordingStatus.textContent = "LIVE AUDIO CAPTURED · READY TO PLAY"; renderAudio(); if (pendingAudioUpload && state.incidentId) { pendingAudioUpload = false; uploadAudioEvidence(state.incidentId, liveRecordingBlob); } } liveRecordingStream?.getTracks().forEach((track) => track.stop()); liveRecordingStream = undefined; }; liveRecorder.start(); refs.liveRecordingTitle.textContent = "Recording live audio"; refs.liveRecordingDuration.textContent = "REC"; refs.liveRecordingStatus.textContent = "MICROPHONE ACTIVE · CAPTURING SOS AUDIO";
...
async function uploadAudioEvidence(alertId, blob) { if (!blob || !alertId) return; log("Uploading audio evidence..."); const formData = new FormData(); formData.append("file", blob, "sos-recording.webm"); formData.append("description", "Emergency SOS audio recording"); try { const response = await fetch(`${CONFIG.API_BASE_URL}/api/alerts/${encodeURIComponent(alertId)}/evidence/audio`, { method: "POST", body: formData }); if (!response.ok) { const body = await response.json().catch(() => ({})); throw new Error(body.error?.message || `HTTP ${response.status}`); } const result = await response.json(); log(`Audio evidence uploaded · SHA-256 verified`); return result; } catch (error) { log(`Audio upload failed · ${error.message}`); return null; } }
