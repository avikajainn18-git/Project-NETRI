const CONFIG = {
  ...
  API_TIMEOUT_MS: 5000,
  LIVE_RECORDING_DURATION_MS: 15000,
};
...
const API_PATHS = {
  ...
  userResponse: (alertId) => `/api/alerts/${encodeURIComponent(alertId)}/user-response`,
};
...
async function sendTrustedContactNotification({ contact, incidentId, backendUrl }) {
  return {
    ...
    phone: contact.phone,
    incidentId,
    status: "SMS (DEMO) SENT",
    detail: "Demo SMS delivered to this trusted contact",
    backendUrl,
  };
}
...
let pendingAudioUpload = false;
async function startLiveRecording() { if (!navigator.mediaDevices?.getUserMedia || !window.MediaRecorder) { refs.liveRecordingStatus.textContent = "LIVE MICROPHONE NOT SUPPORTED IN THIS BROWSER"; return; } try { discardLiveRecording = false; liveRecordingStream = await navigator.mediaDevices.getUserMedia({ audio: true }); liveRecordingChunks = []; liveRecorder = new MediaRecorder(liveRecordingStream); liveRecorder.ondataavailable = (event) => { if (event.data.size) liveRecordingChunks.push(event.data); }; liveRecorder.onstop = () => { const shouldDiscard = discardLiveRecording; const blob = new Blob(liveRecordingChunks, { type: liveRecorder.mimeType || "audio/webm" }); liveRecordingBlob = blob; if (!shouldDiscard) { refs.liveRecordingCard.classList.remove("hidden"); refs.liveRecordingTitle.textContent = "Recording captured"; refs.liveRecordingDuration.textContent = `${Math.max(1, Math.round(blob.size / 1000))} KB`; refs.liveRecordingStatus.textContent = "Audio recorded · Preserving evidence"; if (pendingAudioUpload && state.incidentId) { pendingAudioUpload = false; uploadAudioEvidence(state.incidentId, liveRecordingBlob); } } liveRecordingStream?.getTracks().forEach((track) => track.stop()); liveRecordingStream = undefined; }; liveRecorder.start(); refs.liveRecordingTitle.textContent = "Recording live audio"; refs.liveRecordingDuration.textContent = "REC"; refs.liveRecordingStatus.textContent = "MICROPHONE ACTIVE · CAPTURING SOS AUDIO";
liveRecordingTimer = setTimeout(stopLiveRecording, CONFIG.LIVE_RECORDING_DURATION_MS); } catch (error) { refs.liveRecordingStatus.textContent = "MICROPHONE ACCESS DENIED · LIVE AUDIO UNAVAILABLE"; log("Microphone permission was not granted"); } }
...
function showDemoAlert(incidentId) { if (!state.contacts.length || !refs.demoAlert) return; playAlertPing(); refs.demoAlert.innerHTML = `<strong>SMS DEMO SENT TO ${state.contacts.length} CONTACTS</strong><span>${state.contacts.map((contact) => `${contact.name} · ${contact.phone}`).join("<br />")}</span><small>SOS ${incidentId} · simulated SMS delivery</small>`; refs.demoAlert.classList.remove("hidden"); setTimeout(() => refs.demoAlert.classList.add("hidden"), 7000); }
async function activateSOS() { state.state = DEVICE_STATES.SOS_ACTIVE; state.incidentTime = new Date().toLocaleTimeString(); state.incidentId = `SC-001-${String(Date.now()).slice(-4)}`; moveGps(); state.alert = null; state.delivery = await Promise.all(state.contacts.map((contact) => sendTrustedContactNotification({ contact, incidentId: state.incidentId, backendUrl: CONFIG.API_BASE_URL }))); showDemoAlert(state.incidentId); state.contacts.forEach((contact) => state.notifications.unshift(`SMS (DEMO) SENT TO ${contact.name} · ${contact.phone} · ${new Date().toLocaleTimeString()}`)); localStorage.setItem("safe-compact-notifications", JSON.stringify(state.notifications)); log(`SOS locked in · demo SMS sent to ${state.contacts.length} trusted contacts`); state.hash = await hashEvidence(state.incidentId); const payload = { deviceId: state.deviceId, triggeredAt: new Date().toISOString(), latitude: state.location?.latitude ?? Number(route[state.routeIndex].lat), longitude: state.location?.longitude ?? Number(route[state.routeIndex].lng), batteryLevel: state.battery, signalStatus: effectiveNetwork(), trustedContacts: state.contacts }; state.pendingAlert = payload; savePendingAlert(payload); render(); await transmit(payload); }
async function transmit(payload) { if (effectiveNetwork() === "OFFLINE") { log("Alert queued · waiting for connection"); render(); return; } try { const response = await api.sendAlert(payload); const alert = response.alert || response; state.alert = alert; state.pendingAlert = null; state.incidentId = alert.alert_id || state.incidentId; clearPendingAlert(); state.serverStatus = "RECEIVED"; state.notifications.unshift(`ACTIVE EMERGENCY · ${state.incidentId} · ${new Date().toLocaleTimeString()}`); state.delivery = await Promise.all(state.contacts.map((contact) => sendTrustedContactNotification({ contact, incidentId: state.incidentId, backendUrl: CONFIG.API_BASE_URL }))); localStorage.setItem("safe-compact-notifications", JSON.stringify(state.notifications)); const record = { id: state.incidentId, timestamp: state.incidentTime, deviceId: state.deviceId, location: route[state.routeIndex].place, routeIndex: state.routeIndex, hash: state.hash, deviceState: "ACKNOWLEDGED", status: "PRESERVED", verified: false, access: state.contacts.map((contact) => ({ name: contact.name, phone: contact.phone, status: "AUTHORIZED" })) }; if (!state.incidents.some((x) => x.id === record.id)) state.incidents.unshift(record); localStorage.setItem("safe-compact-incidents", JSON.stringify(state.incidents)); state.state = DEVICE_STATES.ACKNOWLEDGED; log(`Backend acknowledged alert · ${state.incidentId}`); render(); if (liveRecordingBlob) { uploadAudioEvidence(state.incidentId, liveRecordingBlob); } else { pendingAudioUpload = true; } } catch (error) { state.serverStatus = "UNREACHABLE"; log(`Server connection failed · alert remains queued`); render(); } }
...
async function uploadAudioEvidence(alertId, blob) { if (!blob || !alertId) return; refs.liveRecordingStatus.textContent = "Uploading evidence..."; refs.liveRecordingTitle.textContent = "Uploading evidence"; log("Uploading audio evidence..."); const formData = new FormData(); formData.append("file", blob, "sos-recording.webm"); formData.append("description", "Emergency SOS audio recording"); try { const response = await fetch(`${CONFIG.API_BASE_URL}/api/alerts/${encodeURIComponent(alertId)}/evidence/audio`, { method: "POST", body: formData }); if (!response.ok) { const body = await response.json().catch(() => ({})); throw new Error(body.error?.message || `HTTP ${response.status}`); } const result = await response.json(); refs.liveRecordingTitle.textContent = "Evidence secured"; refs.liveRecordingStatus.textContent = "Audio preserved in evidence vault"; log(`Audio evidence uploaded · SHA-256 verified`); return result; } catch (error) { refs.liveRecordingTitle.textContent = "Upload failed"; refs.liveRecordingStatus.textContent = `Upload error · ${error.message}`; log(`Audio upload failed · ${error.message}`); return null; } }
...
async function sendUserResponse() { const alertId = state.incidentId; if (!incidentExists() || userResponseSent || !state.alert || !alertId) return; userResponseSent = true; refs.responseBtn.disabled = true; refs.responseBtn.textContent = "SENDING"; try { const response = await request(API_PATHS.userResponse(alertId), { method: "POST", body: JSON.stringify({ responseType: "COMPACT_BUTTON" }) }); refs.responseBtn.textContent = "RESPONSE SENT"; refs.responseBtn.classList.add("responded"); log(`User response sent · ${alertId}`); refs.responseStatus.textContent = "RESPONSE"; refs.responseBody.innerHTML = `<strong>SOS SENT ✓</strong><br />Incident ID: ${alertId}<br />User responded: ${response.response?.response_type || 'COMPACT_BUTTON'}<br />Server: ${state.serverStatus}`; } catch (error) { userResponseSent = false; refs.responseBtn.disabled = false; refs.responseBtn.textContent = "RESPONSE"; log(`Response failed · ${error.message}`); } }
function resetResponseState() { userResponseSent = false; if (refs.responseBtn) { refs.responseBtn.disabled = true; refs.responseBtn.textContent = "RESPONSE"; refs.responseBtn.classList.remove("responded"); refs.responseBtn.setAttribute("aria-label", "Send user response"); } }
async function resetDemo() { const batteryLevel = state.battery; state.state = DEVICE_STATES.RESETTING; render(); clearInterval(countdownTimer); stopLiveRecording({ discard: true }); geolocation.stop(); taps.reset(); try { await api.reset(state.deviceId, { device_id: state.deviceId, timestamp: new Date().toISOString(), reason: "MANUAL_RESET" }); } catch {} state.state = effectiveNetwork() === "OFFLINE" ? DEVICE_STATES.DEVICE_OFFLINE : DEVICE_STATES.IDLE; state.battery = batteryLevel; state.alert = null; state.pendingAlert = null; state.incidentId = null; state.incidentTime = null; state.hash = "Generated after SOS activation"; state.verified = false; state.tapCount = 0; state.movement = []; clearPendingAlert(); resetResponseState(); log("Demo reset · server evidence is not deleted"); geolocation.start(); render(); }
