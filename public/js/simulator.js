...
function speakDemoInteraction(text, button) { if (!window.speechSynthesis) return; window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); utterance.rate = 0.92; utterance.pitch = 1; utterance.onend = () => { button.textContent = "▶"; }; window.speechSynthesis.speak(utterance); button.textContent = "Ⅱ"; }
function renderAudio() { document.querySelectorAll(".play-button").forEach((button) => { button.onclick = () => { const audio = button.closest(".audio-card")?.querySelector("audio"); if (!audio) return; if (!audio.src || audio.error?.code === MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED) { speakDemoInteraction(button.dataset.demoText, button); return; } if (audio.paused) { document.querySelectorAll("audio.evidence-audio").forEach((item) => item !== audio && item.pause()); void audio.play(); button.textContent = "Ⅱ"; } else { audio.pause(); button.textContent = "▶"; } audio.onended = () => { button.textContent = "▶"; }; }; }); }
...
function beginCountdown() { state.state = DEVICE_STATES.COUNTDOWN; state.countdown = 3; log("Triple tap validated · countdown started"); refs.countdownValue.textContent = "3"; clearInterval(countdownTimer); countdownTimer = setInterval(() => { state.countdown -= 1; if (state.countdown <= 0) { clearInterval(countdownTimer); activateSOS(); } render(); }, 1000); render(); }
function cancelCountdown() { clearInterval(countdownTimer); state.state = DEVICE_STATES.IDLE; state.tapCount = 0; state.countdown = null; log("Countdown cancelled · no alert transmitted"); }
...
async function activateSOS() { state.state = DEVICE_STATES.SOS_ACTIVE; state.incidentTime = new Date().toLocaleTimeString(); state.incidentId = `SC-001-${String(Date.now()).slice(-4)}`; state.alert = null; showDemoAlert(state.incidentId); state.notifications.unshift(`DEMO ALERT SENT TO SISTER · ${state.contacts.find((contact) => contact.name === "Sister")?.phone || ""} · ${new Date().toLocaleTimeString()}`); localStorage.setItem("safe-compact-notifications", JSON.stringify(state.notifications)); log("SOS locked in · demo alert sent to Sister"); state.hash = await hashEvidence(state.incidentId); const payload = { device_id: state.deviceId, trigger: "TRIPLE_TAP", location: state.location?.source === "REAL" ? "REAL GPS" : route[state.routeIndex].place, lat: state.location?.latitude ?? Number(route[state.routeIndex].lat), lng: state.location?.longitude ?? Number(route[state.routeIndex].lng), channel: effectiveNetwork() === "OFFLINE" ? "GSM" : "INTERNET", contactsTotal: state.contacts.length, timestamp: new Date().toISOString(), battery: state.battery, network: effectiveNetwork(), deviceState: "SOS_ACTIVE", idempotencyKey: `${state.deviceId}_${Date.now()}` }; state.pendingAlert = payload; savePendingAlert(payload); render(); await transmit(payload); }
async function transmit(payload) { if (effectiveNetwork() === "OFFLINE") { log("Alert queued · waiting for connection"); render(); return; } try { const response = await api.sendAlert(payload); state.alert = response; state.pendingAlert = null; state.incidentId = response.incidentId || response.alertId || response.id || state.incidentId; clearPendingAlert(); state.serverStatus = "RECEIVED"; state.notifications.unshift(`ACTIVE EMERGENCY · ${state.incidentId} · ${new Date().toLocaleTimeString()}`); state.delivery = await Promise.all(state.contacts.map((contact) => sendTrustedContactNotification({ contact, incidentId: state.incidentId, backendUrl: CONFIG.API_BASE_URL }))); localStorage.setItem("safe-compact-notifications", JSON.stringify(state.notifications)); const record = { id: state.incidentId, timestamp: state.incidentTime, deviceId: state.deviceId, location: route[state.routeIndex].place, routeIndex: state.routeIndex, hash: state.hash, deviceState: "ACKNOWLEDGED", status: "PRESERVED", verified: false, access: state.contacts.map((contact) => ({ name: contact.name, phone: contact.phone, status: "AUTHORIZED" })) }; if (!state.incidents.some((x) => x.id === record.id)) state.incidents.unshift(record); localStorage.setItem("safe-compact-incidents", JSON.stringify(state.incidents)); state.state = DEVICE_STATES.ACKNOWLEDGED; log(`Backend acknowledged alert · ${state.incidentId}`); render(); } catch (error) { state.serverStatus = "UNREACHABLE"; log(`Server connection failed · alert remains queued`); render(); } }
...
async function loseDevice() { if (!incidentExists()) { log("Device loss requires an active incident"); return; } state.state = DEVICE_STATES.DEVICE_LOST; geolocation.stop(); log("Device marked lost · cloud evidence remains available"); render(); }
async function resetDemo() { const batteryLevel = state.battery; state.state = DEVICE_STATES.RESETTING; render(); clearInterval(countdownTimer); geolocation.stop(); taps.reset(); try { await api.reset(state.deviceId, { device_id: state.deviceId, timestamp: new Date().toISOString(), reason: "MANUAL_RESET" }); } catch {} state.state = effectiveNetwork() === "OFFLINE" ? DEVICE_STATES.DEVICE_OFFLINE : DEVICE_STATES.IDLE; state.battery = batteryLevel; state.alert = null; state.pendingAlert = null; state.incidentId = null; state.incidentTime = null; state.hash = "Generated after SOS activation"; state.verified = false; state.tapCount = 0; state.movement = []; clearPendingAlert(); log("Demo reset · server evidence is not deleted"); geolocation.start(); render(); }
...
function switchTab(tab) { document.querySelectorAll(".page").forEach((page) => page.classList.add("hidden")); $(`${tab}Page`).classList.remove("hidden"); document.querySelectorAll(".nav-item").forEach((item) => item.classList.toggle("nav-active", item.dataset.tab === tab)); refs.breadcrumb.textContent = tab === "overview" ? "COMMAND CENTER" : tab === "vault" ? "EVIDENCE VAULT" : "TRUSTED CONTACTS"; }

refs.compact.onclick = triggerTap; refs.compact.onkeydown = (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); triggerTap(); } }; refs.cancelCountdown.onclick = cancelCountdown; refs.triggerSos.onclick = () => { if (state.state === DEVICE_STATES.IDLE) [0, 1, 2].forEach((_, i) => setTimeout(triggerTap, i * 220)); }; refs.resetDemo.onclick = resetDemo; $("moveGps").onclick = moveGps; $("verifyButton").onclick = verifyIntegrity; $("addContact").onclick = addContact; $("vaultLose").onclick = loseDevice; $("openEvidence").onclick = () => switchTab("vault"); $("controllerReset").onclick = resetDemo; $("controllerTrigger").onclick = () => [0, 1, 2].forEach((_, i) => setTimeout(triggerTap, i * 220)); $("controllerMove").onclick = moveGps; $("controllerLost").onclick = loseDevice; $("controllerVerify").onclick = verifyIntegrity; refs.tapWindow.oninput = (e) => { state.tapWindow = Number(e.target.value); log(`Tap window set to ${(state.tapWindow / 1000).toFixed(1)} seconds`); };
